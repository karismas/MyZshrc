...
function av() {
	...
	for i in $(seq 1 $num);
	do
		res=$(source "${c}.venv/bin/activate" 2>&1)
		if [ "$res" = "" ]; then
...
function addsub() {
	if [ $# -eq 0 ]; then
		...
		FILE=$1
		FILE="${FILE%%.*}"
		ffmpeg -i $FILE.mp4 -i $FILE.en.srt -c copy -c:s mov_text -metadata:s:s:0 language=eng out.mp4
		rm $FILE.mp4
		rm $FILE.en.srt
		mv out.mp4 $FILE.mp4
	fi
...
function addsubdir() {
	find . -name "*.srt" | while read -r file
	do
		addsub "${file:2}"
	done
}
...
function domv() {
	...
	unzip Files.zip
	rm Files.zip
	mv Files "Programming Assignment"
	mv Programming\ Assignment/*/* Programming\ Assignment
	find Programming\ Assignment -type d ! -name "data" ! -name Programming\ Assignment | while read -r file
	do
		rm -r "$file"
	done
}
